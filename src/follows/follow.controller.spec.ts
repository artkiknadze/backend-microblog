...
describe('FollowsController', () => {
  ...
  it('/:userId (POST) should follow a user', async () => {
    const userId = 1;
    const followedId = 2;
    const follow = controller.create(followedId.toString(), {
      user: { sub: userId },
    });
  ...
  it('/:userId (DELETE) should unfollow a user', async () => {
    ...
    jest
      .spyOn(controller['followsService']['followsRepository'], 'findOne')
      .mockResolvedValueOnce(followMock as any);
    const unfollow = await controller.remove(followedId.toString(), {
      user: { sub: userId },
    });
